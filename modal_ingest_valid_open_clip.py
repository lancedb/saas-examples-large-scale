...
def process_batch(batch_data):
    ...
    images = []
    valid_indices = []
    valid_image_bytes = [None] * len(batch_data)  # Initialize with None values
        ...
        if img_bytes:
            try:
                ...
                try:
                    ...
                    img.verify() 
                    valid_image_bytes[i] = img_bytes  # Store valid image at original index
                except Exception as e:
    ...
    records = []
    for i, img_bytes in enumerate(valid_image_bytes):
        if img_bytes is not None:  # Only process records with valid images
            try:
                record = batch_data[i]
                records.append({
    ...
    try:
        ...
        tbl_name = "artworks-siglip2-base-embed-api"
