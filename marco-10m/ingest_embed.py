...
MODEL_DIR = "/cache"
BATCH_SIZE = 200000
EMBEDDING_BATCH_SIZE = 4096  # Smaller batches for GPU memory
...
class EmbeddingProcessor:
    def __init__(self):
        ...
        tbl_name = "10M-marco-siglip2-class-full"
        try:
    ...
    def process_batch(self, batch_args: dict):
        ...
        def process_single_record(record):
            try:
                # Convert image bytes to PIL Image and preprocess in one go
                img_bytes = io.BytesIO(record["image"]["bytes"])
                img_tensor = self.preprocess(Image.open(img_bytes).convert('RGB')).unsqueeze(0)
        ...
        for embed_chunk in chunk_records(processed_records, EMBEDDING_BATCH_SIZE):
            try:
                ...
                images = []
                valid_records = []
                for img_tensor, record in results:
