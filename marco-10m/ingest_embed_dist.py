...
MODEL_DIR = "/cache"
BATCH_SIZE = 200000
EMBEDDING_BATCH_SIZE = 8000  # Smaller batches for GPU memory
...
# MODEL_PRETRAINED = f"{MODEL_DIR}/models--timm--ViT-L-16-SigLIP2-256/snapshots/da9426945c7c5acbd3afd0c158c8bb9cfd4d8cc0/open_clip_pytorch_model.bin"
...
# Update in EmbeddingGenerator class
...
# Modify the EmbeddingProcessor class
@stub.cls(
    image=image,
    timeout=86400,
    memory=100000,
...
# Update in EmbeddingProcessor class
    ...
    def process_batch(self, batch_args: dict):
        ...
        for embed_chunk in chunk_records(processed_records, EMBEDDING_BATCH_SIZE):
            try:
                ...
                # Generate embeddings using the separate class
                t1 = time.time()
                embeddings = self.embedding_generator.generate_embeddings.remote(images)
